...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("moveit_servo.hand_tracking_demo");
...
int main(int argc, char** argv)
{
  ...
  rclcpp::Node::SharedPtr node = rclcpp::Node::make_shared("hand_tracking_demo");
  ...
  auto init_pose = perception_listener.getLatestPose();
  RCLCPP_INFO(LOGGER, "Initial target pose: "
  ...
  target_pose_pub->publish(*init_pose);
  ...
  while (rclcpp::ok())
  {
    ...
    target_pose.pose.position.x = latest_pose->pose.position.x; // original x is pixel value in 480x480 image
    target_pose.pose.position.y = 0.0; // since robot only moves in XZ plane, i.e. Y always 0
